...
import torch
import os
def plot_distributions(pred_dis: torch.Tensor, 
                           gt_distribution: torch.Tensor, 
                           batch_id: int = 0, 
                           box_indices: list = [0],
                           save_dir: str = "./box_dist_plots"):
    """
    可视化预测分布和真实分布（针对检测框），并保存到文件。

    Args:
        pred_dis (Tensor): 预测分布, shape [B, N, bins]
        gt_distribution (Tensor): 真实分布, shape [B, N, bins]
        batch_id (int): 选择的 batch 索引
        box_indices (list): 要画的 box 索引列表
        save_dir (str): 保存文件夹
    """
    assert pred_dis.shape == gt_distribution.shape, "预测分布和真实分布形状必须一致"
    N, bins = pred_dis.shape
    ...
    for box_id in box_indices:
        assert box_id < N, f"box {box_id} 超出范围 (最大 {N-1})"
        pred = pred_dis[box_id].detach().cpu().numpy()
        gt   = gt_distribution[box_id].detach().cpu().numpy()

        plt.figure(figsize=(8,5))
        plt.plot(x, pred, label="Predicted", marker="o")
        plt.plot(x, gt, label="Ground Truth", marker="x")
        plt.title(f"Depth Distribution (Batch {batch_id}, Box {box_id})")
        plt.xlabel("Bin index")
        plt.ylabel("Probability")
        plt.legend()
        plt.grid(True)

        save_path = os.path.join(save_dir, f"depth_dist_b_box{box_id}.png")
