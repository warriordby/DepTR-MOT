...
######################################################################
        self.sampling_offsets = nn.Linear(embed_dim, self.total_points * 3)
######################################################################
    ...
    def _reset_parameters(self):
        ...
        grid_init *= scaling
#######################################################################
        depth_scaling = torch.linspace(-1, 1, sum(self.num_points_list)).reshape(1, -1, 1)  # 从近到远或远到近
...
#################################################################################
        sampling_offsets = sampling_offsets.reshape(
            bs, Len_q, self.num_heads, sum(self.num_points_list), 3
        )
#################################################################################
        ...
        if reference_points.shape[-1] == 2:
            ...
            box_points= reference_points[:, :, None, :, :2] + base_offset

###################################
            depth = sampling_offsets[:, :, :, :,2] + reference_points[:, :, None, :, 4]

            # 最终采样位置 = 中心点 + 深度调制后的偏移
            sampling_locations = torch.cat([box_points,depth.unsqueeze(-1)],dim=-1)
        else:
            raise ValueError(
...
class Integral(nn.Module):
    ...
    def forward(self, x, project):
        ...
        x = F.linear(x, project.to(x.device)).reshape(-1, 5)
...
class LQE(nn.Module):
    def __init__(self, k, hidden_dim, num_layers, reg_max):
        ...
        self.k = k
        self.reg_max = reg_max
        self.reg_conf = MLP(5 * (k + 1), hidden_dim, 1, num_layers)
        init.constant_(self.reg_conf.layers[-1].bias, 0)
        init.constant_(self.reg_conf.layers[-1].weight, 0)

    def forward(self, scores, pred_corners):
        B, L, _ = pred_corners.size()
        prob = F.softmax(pred_corners.reshape(B, L, 5, self.reg_max + 1), dim=-1)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.lqe_layers = nn.ModuleList(
            [copy.deepcopy(LQE(5, 64, 2, reg_max)) for _ in range(num_layers)]
        )

    def value_op(self, memory, value_proj, value_scale, memory_mask, memory_spatial_shapes):
        """
        输出为 多个张量组成的列表，每个张量对应 memory 中一个尺度的 value 特征，格式为：
        [value_1, value_2, ..., value_N]（N 为 memory_spatial_shapes 的长度）

        每个 value_i 的形状为：
        [B, num_head, head_dim, H_i * W_i]
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ref_points_input = ref_points_detach.unsqueeze(2) #torch.Size([2, 500, 1, 5])
            query_pos_embed = query_pos_head(ref_points_detach).clamp(min=-10, max=10) #torch.Size([2, 500, 256])
...
#################################################################################原模型创新，将边界框与角点距离进行编码和解码，对于深度值分别进行处理
            ...
            inter_ref_bbox_with_depth=torch.cat([inter_ref_bbox,ref_points_initial[:,:,-1:]], dim=-1)
###############################################################################
            if self.training or i == self.eval_idx:
                ...
                dec_out_bboxes.append(inter_ref_bbox_with_depth)
            ...
            ref_points_detach = inter_ref_bbox_with_depth.detach()
            output_detach = output.detach()
...
class TransformerDecoderFD(nn.Module):
    """
    Transformer Decoder for depth.
    ...
    def __init__(
        ...
        super(TransformerDecoderFD, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.layer_scale = layer_scale
        self.num_head = num_head
    ...
    def forward(
        ...
        spatial_shapes,
        depth_head,  # 深度预测头（核心）
        score_head,  # 深度预测的置信度头
        query_pos_head,  # 深度查询的位置嵌入头
        pre_depth_head,  # 初始深度预测头
        integral,
        attn_mask=None,
        memory_mask=None,
        dn_meta=None,
        ...
        # 初始化：输入目标特征作为解码器初始输出
        output = target
        output_detach = 0  # 用于存储detach的中间特征，避免梯度连锁
        # 预处理注意力机制的value特征（适配多尺度输入）
        value = self.value_op(memory, None, None, memory_mask, spatial_shapes)

        # 存储各层的深度预测结果
        dec_out_depths = []  # 多层深度预测值
        dec_out_confidence = []  # 多层深度预测的置信度
        dec_out_initial_refs = []  # 初始参考点（用于追溯迭代起点）

        # 激活初始参考点（将原始参考点转为可解释的数值范围）
        ref_points_detach = F.sigmoid(ref_points_unact)
        ...
        for i, layer in enumerate(self.layers):
            ...
            # 生成深度查询的位置嵌入（融合空间位置信息）
            query_pos_embed = query_pos_head(ref_points_detach).clamp(min=-10, max=10)

            # 宽层尺度调整（针对高分辨率深度特征的细化）
            if i >= self.eval_idx + 1 and self.layer_scale > 1:
                query_pos_embed = F.interpolate(query_pos_embed, scale_factor=self.layer_scale)
                value = self.value_op(
                    memory, None, query_pos_embed.shape[-1], memory_mask, spatial_shapes
                ...
                output = F.interpolate(output, size=query_pos_embed.shape[-1])
                output_detach = output.detach()  # 冻结当前特征，避免梯度影响后续迭代

            # 通过解码器层更新特征（融合注意力机制优化深度特征表示）
            output = layer(
                output, ref_points_input, value, spatial_shapes, attn_mask, query_pos_embed
            )

            # 第一层：生成初始深度预测（作为后续迭代的基准）
            if i == 0:
                # 初始深度预测（结合逆sigmoid操作细化参考点）
                pre_depths = F.sigmoid(pre_depth_head(output) + inverse_sigmoid(ref_points_detach))
                # 初始深度预测的置信度
                pre_confidence = score_head[0](output)
                # 保存初始深度参考点（用于后续迭代的基准）
                initial_depth_refs = pre_depths.detach()

            #################################################################################
            # 核心：通过深度头预测当前层的深度值（迭代细化）
            # 融合当前特征与上一层detach特征，避免梯度冲突
            pred_depths = depth_head[i](output + output_detach) 
            # 直接将预测结果作为当前层的深度输出
            current_depth = pred_depths
            #################################################################################

            # 收集结果（训练时保留所有层，推理时只保留目标层）
            if self.training or i == self.eval_idx:
                # 当前层深度预测的置信度
                depth_confidence = score_head[i](output)
                # 存储当前层结果
                dec_out_confidence.append(depth_confidence)
                dec_out_depths.append(current_depth)
                dec_out_initial_refs.append(initial_depth_refs)

                # 推理阶段：达到目标层后停止迭代
                if not self.training:
                    break

            # 更新参考点（用当前层深度预测结果作为下一层的输入参考）
            ref_points_detach = current_depth.detach()
            # 更新detach特征（下一层迭代的特征基准）
            output_detach = output.detach()

        # 返回深度预测相关结果
        return (
            torch.stack(dec_out_depths),  # 所有层的深度预测值
            torch.stack(dec_out_confidence),  # 所有层的深度置信度
            torch.stack(dec_out_initial_refs),  # 初始深度参考点
            pre_depths,  # 第一层的初始深度预测
            pre_confidence,  # 初始深度预测的置信度
        )
@register()
...
######################################################################
        ddecoder_layer = TransformerDecoderLayer(
            hidden_dim,
            nhead,
            dim_feedforward,
            dropout,
        ...
        ddecoder_layer_wide = TransformerDecoderLayer(
            hidden_dim,
            nhead,
            dim_feedforward,
            dropout,
        ...
        self.depth_decoder = TransformerDecoderFD(
            hidden_dim,
            ddecoder_layer,
            ddecoder_layer_wide,
            num_layers,
...
###############################################################################
        self.pre_depth_head = MLP(hidden_dim, hidden_dim, 4, 3)
