...
from VideoDepthAnything.utils.util import compute_scale_and_shift, get_interpolate_frames
...
class VideoDepthAnything(nn.Module):
    ...
    def forward(self, x):
        ...
        # 参考建议的比例限制，调整输入尺寸
        input_size = max(frame_height, frame_width)
        if ratio > 1.78:  # 16:9 ≈ 1.78
            input_size = int(input_size * 1.777 / ratio)
            input_size = round(input_size / 14) * 14  # 确保是14的倍数
        
        # 确保最小尺寸是14的倍数
        target_h = round(frame_height / 14) * 14
        target_w = round(frame_width / 14) * 14
        
        # 如果需要缩放
        if (target_h, target_w) != (frame_height, frame_width):
            # 调整输入形状以适应F.interpolate的要求：[B*T, C, H, W]
            x_reshaped = x.flatten(0, 1)  # 合并B和T维度
            # 缩放至目标尺寸
        ...
        depth, _, fea = self.head(features, patch_h, patch_w, T)
        ...
        # 调整输出形状并返回
        return depth.squeeze(1).unflatten(0, (B, T)).detach(), fea # return shape [B, T, H, W]
